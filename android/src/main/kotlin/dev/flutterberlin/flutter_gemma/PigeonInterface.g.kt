...
interface PlatformService {
  ...
  fun createSession(temperature: Double, randomSeed: Long, topK: Long, topP: Double?, loraPath: String?, enableVisionModality: Boolean?, enableAudioModality: Boolean?, systemInstruction: String?, toolDefinitionsJson: List<String>?, enableThinking: Boolean?, callback: (Result<Unit>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val toolDefinitionsJsonArg = args[8] as List<String>?
            val enableThinkingArg = args[9] as Boolean?
            api.createSession(temperatureArg, randomSeedArg, topKArg, topPArg, loraPathArg, enableVisionModalityArg, enableAudioModalityArg, systemInstructionArg, toolDefinitionsJsonArg, enableThinkingArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
